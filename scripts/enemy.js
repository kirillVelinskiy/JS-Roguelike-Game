...
// Содзание противников на основе наших параметров (определенно в settings.js)
...
function spawnEnemy(enemyParameters) {
  ...
  enemyArray[enemyId]['getDamage'] = function (damage) {
    ...
    $('.field')
      ...
      .eq(this.x)
      .children(':first')
      .css({ width: (this.hp / this.maxhp) * 100 + '%' });
    if (this.hp <= 0) {
  ...
  $('<div>', { class: 'health' })
    .css({
      width: (enemyArray[enemyId].hp / enemyArray[enemyId].maxhp) * 100 + '%',
    })
    .appendTo(
      $('.field')
        .children()
        .eq(enemyArray[enemyId].y)
        .children()
        .eq(enemyArray[enemyId].x)
    );
  setTimeout(() => {
    enemyAction(enemyId);
  }, 1000);
...
// Логика дейтвий противника
function enemyAction(enemyId) {
  let enemy = enemyArray[enemyId];

  // Рекурсия заканчивается после смерти противника
  if (enemy.hp <= 0) {
  ...
  // Если игрок в области зрения, противник идет к нему
  ...
  // Если предыдущие условия не выполнены, противник просто передвигается в рандомном направлении на 1 клетку
  else {
    const directons = [
      [-1, 0],
      [0, -1],
...
// Перемещение противника на указанные координаты
...
// Проверка на обнаружение игрока в области видимости противника
